...
Our interest is in $\mathbb{E}[Y|X]$ and we aim to find that the investigational arm is:
...
An ordinal cumulative logistic regression model is used to analyse the data. The model assumes that the distribution of DWS in the population is smooth (e.g. the examples below).
