...
Suppose that days with symptoms is distributed as in @fig-dist-days-with-symptoms-example01.
The first 100 participants are randomly assigned to one of three interventions. Sample data is shown in @fig-observed-data-example01.
An ordinal regression model for DWS is inferred from the data which assumes a proportional effect on the cumulative odds of having a lower/higher DWS.

The modelled distribution for DWS for each intervention group are in @fig-modelled-dist-dwsg-example01.
The posterior mean difference in DWS is in @fig-posterior-difference-in-means-example01.
...
#|      Open points indicate observed sample proportions in each outcome level.
#| fig-height: 6
#| fig-width: 6
p3
```
...
Sample data is shown in @fig-observed-data-example02.

The modelled distribution for DWS for each intervention group are in @fig-modelled-dist-dwsg-example02.
The posterior mean difference in DWS is in @fig-posterior-difference-in-means-example02.
...
This is a slightly more interesting, albeit perhaps unrealistic, example.
Suppose that almost everyone on standard therapy has very poor ashtma control and so expect very high number of days with symptoms.
The true model does not satisfy proportional cumulative odds assumption.
