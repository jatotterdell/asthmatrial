...
data {
  ...
  real<lower=0> theta_scale; // prior scale on Ispline coefficients
...
transformed parameters {
  ...
  vector[P_Isp] theta = theta_scale * gamma * omega;
...
model {
  ...
  target += std_normal_lpdf(beta_raw) +
            ...
            dirichlet_lpdf(omega | rep_vector(2 * inv(P_Isp), P_Isp));
  // likelihood
  if (!prior) {
    for(n in 1:N)
