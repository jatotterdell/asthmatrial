...
#' @title inv_logit(x)
#' @description Alias for expit(x)
#' @param x The value to transform
#' @return inv_logit(x)
inv_logit <- function(x) expit(x)


#' @title ord_logit
#' @description Calculate ordered logit
#' @param x A vector of probabilities which sum to 1
#' @return Ordered logit of x
ord_logit <- function(x) {
    return(logit(cumsum(x))[-length(x)])
}


#' @title inv_ord_logit
#' @description Calculate inverse ordered logit
#' @param x A vector of non-decreasing real numbers
#' @return Inverse ordered logit of x
...
compile_rstan_mod <- function() {
  rstan::stan_model(file = system.file("stan", "cumulative_logistic_agg_rev.stan", package = "asthmasims"))
}
...
compile_rstan_approx_mod <- function() {
  rstan::stan_model(file = system.file("stan", "cumulative_logistic_agg_rev_altpar.stan", package = "asthmasims"))
}
...
compile_cmdstanr_mod <- function(mod) {
  cmdstanr::cmdstan_model(stan_file = system.file("stan", paste0(mod, ".stan"), package = "asthmasims"))
}
